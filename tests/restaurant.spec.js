...
>>>>>>> a665641a2d05cd8e245d673e7c0dd1ef8cddb40a
    ...
    assert.deepStrictEqual(menu.hasOwnProperty('fetchMenu'), true);
    ...
    // ```
    objetoRetornado.order('coxinha');
    assert.strictEqual(objetoRetornado.consumption.filter(food => food === 'coxinha').length, 2);
    ...
    // ```
    menu = createMenu({
      food: { 'coxinha': 4.50, 'sashimi': 19.90 },
      drink: { 'agua': 3.50, 'sopa': 7.00 },
    });
    menu.order('sashimi');
    menu.order('sopa');
    assert.strictEqual(menu.pay(), 29.59);
