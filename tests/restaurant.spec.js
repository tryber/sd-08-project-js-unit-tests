...
describe('#createMenu', () => {
  it('tests the function has the correct behaviour', () => {
    ...
    const objRetornado1 = createMenu(itens);
    assert.strictEqual(typeof objRetornado1, 'object', 'Objeto esperado é um OBJECT');

    const keyObjRetornado = Object.keys(objRetornado1)[0];
    assert.strictEqual(keyObjRetornado, 'fetchMenu');

    assert.strictEqual(typeof objRetornado1.fetchMenu, 'function');
    const retornoFetchMenu = objRetornado1.fetchMenu();
    assert.strictEqual(typeof retornoFetchMenu, 'object');

    // --------------------------------------------------------------------------------------
    // TESTE 2: Verifique que, quando a função createMenu for chamada passando como parâmetro o objeto: `{ food: {}, drink: {} }`, retorne um objeto que ao executar a função associada a propriedade `fetchMenu` deste objeto, o retorno da função de ser um objeto que tenha somente as chaves `food` e `drinks`.
    // ```
    // const objetoRetornadoTeste2 = createMenu(objetoQualquer);
    // objetoRetornadoTeste2.fetchMenu() // retorno: { food: {}, drink: {}}
    // ```
    const objRetornado2 = createMenu(itens);
    const keysObj = Object.keys(objRetornado2.fetchMenu());
    const keysObjEspected1 = ['food', 'drinks'];
    assert.deepStrictEqual(keysObj, keysObjEspected1);
    ...
    // ```
    const objetoRetornado3 = createMenu(itens);
    const objFechtMenu = objetoRetornado3.fetchMenu();
    assert.deepStrictEqual(objFechtMenu, itens);
    ...
    // ```
    const objetoRetornado4 = createMenu(itens);
    const arrayConsumption = objetoRetornado4.consumption;
    assert.deepStrictEqual(arrayConsumption, []);
    ...
    const objetoRetornadoTeste5 = createMenu(itens);
    objetoRetornadoTeste5.order('coxinha');
    const arrayConsumption2 = objetoRetornadoTeste5.consumption;
    assert.deepStrictEqual(arrayConsumption2, ['coxinha']);
    // ```
    ...
    // ```
    const objetoRetornadoTeste6 = createMenu(itens);
    objetoRetornadoTeste6.order("agua");
    objetoRetornadoTeste6.order("sopa");
    objetoRetornadoTeste6.order("sashimi");
    const isConsum = objetoRetornadoTeste6.consumption;
    assert.deepStrictEqual(isConsum, ["agua", "sopa", "sashimi"]);
    ...
    // ```
    const objetoRetornadoTeste7 = createMenu(itens);
    ...
    // ```
    const objetoRetornadoTeste8 = createMenu(itens);
    objetoRetornadoTeste8.order('agua');
    objetoRetornadoTeste8.order('agua');
    objetoRetornadoTeste8.order('sopa');
    objetoRetornadoTeste8.order('sashimi');
    const somaEspected = 3.90 + 3.90 + 3.90 + 9.90;
    const somaRetornado = objetoRetornadoTeste8.pay();
    assert.strictEqual(somaRetornado, somaEspected);
