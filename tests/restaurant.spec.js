...
describe('#createMenu', () => {
  it('tests the function has the correct behaviour', () => {
    ...
    const test0 = Object.keys(objetoRetornado.fetchMenu);
    assert.deepStrictEqual(test0, ['food', 'drink']);
    // Agora faça o TESTE 3 deste arquivo.
    // --------------------------------------------------------------------------------------
    // TESTE 3: Verifique que o menu passado pra função createMenu é identico ao menu recuperado pela função 'objetoRetornado.fetchMenu'

    // const objetoRetornado = createMenu(objetoQualquer);
    const test1 = objetoRetornado.fetchMenu; // Retorno: objetoQualquer
    assert.deepStrictEqual(test1, { food: {}, drink: {} });
    ...
    // ```
    const test2 = createMenu({});
    test2.order('coxinha');
    test2.order('agua');
    test2.order('sopa');
    test2.order('sashimi');
    // objetoRetornado.consumption // Retorno: ["coxinha", "agua", "sopa", "sashimi"]
    assert.strictEqual(test2.consumption.length, 4);
    // Agora faça o TESTE 7 deste arquivo.
    // --------------------------------------------------------------------------------------
    // TESTE 7: Verifique que a função `order` aceita que pedidos repetidos sejam acrescidos a consumption.
    // ```
    const test3 = createMenu({});
    test3.order('coxinha');
    test3.order('agua');
    test3.order('coxinha');
    // objetoRetornado.comsuption // Retorno: ['coxinha', 'agua', 'coxinha']
    assert.deepStrictEqual(test3.consumption, ['coxinha', 'agua', 'coxinha']);
    // Agora faça o TESTE 8 deste arquivo.
    // --------------------------------------------------------------------------------------
    // TESTE 8: Verifique que, ao chamar `objetoRetornado.pay()`, retorna-se a soma dos preços de tudo que foi pedido, conforme registrado em `objetoRetornado.consumption`
    const test4 = createMenu({ food: { coxinha: 3.9, sopa: 9.9 }, drink: { agua: 3.9, cerveja: 6.9 } });
    test4.order('coxinha');
    test4.order('agua');
    test4.order('coxinha');
    //objetoRetornado.pay() // Retorno: somaDosPreçosDosPedidos 12.09
    assert.strictEqual(test4.pay(), 12.87);
