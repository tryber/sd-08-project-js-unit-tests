...
describe("#createMenu", () => {
  it("tests the function has the correct behaviour", () => {
    ...
    const paramentEmpty = { food: {}, drink: {} };
    const restaurantEmpty = createMenu(paramentEmpty);

    const objectfetMenu = {
      fetchMenu: {},
    };

    assert.deepStrictEqual(typeof restaurantEmpty, "object");
    assert(restaurantEmpty, objectfetMenu);

    // passo 2
    assert.deepStrictEqual(restaurantEmpty.fetchMenu, paramentEmpty);
    ...
    const restaurant = createMenu(parament);

    assert.deepStrictEqual(restaurant.fetchMenu, parament);
    ...
    restaurantOrder.order("agua");
    restaurantOrder.order("sopa");
    restaurantOrder.order("sashimi");
    const restaurantOrderLength = Object.values(restaurantOrder.consumption)
      .length;
    ...
    assert.deepStrictEqual(restaurantOrderLength, 4);
  });

  // passo 7
  const restaurantOrderClone = createMenu({
    food: { coxinha: 3.9, sopa: 9.9 },
    drink: { agua: 3.9, cerveja: 6.9 },
  });
  restaurantOrderClone.order("coxinha");
  restaurantOrderClone.order("agua");
  restaurantOrderClone.order("coxinha");

  assert.deepStrictEqual(restaurantOrderClone.consumption, [
    "coxinha",
    "agua",
    "coxinha",
  ]);
  ...
  // passo 8
  const restaurantPay = createMenu({
    food: { coxinha: 3.9, sopa: 9.9 },
    drink: { agua: 3.9, cerveja: 6.9 },
  });
  restaurantPay.order("coxinha");
  restaurantPay.order("coxinha");
  restaurantPay.order("coxinha");

  assert.strictEqual(restaurantPay.pay(), 11.7);
});
