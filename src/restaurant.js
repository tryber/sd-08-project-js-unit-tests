...
/*
  ...
  através desse sistema, cadastrar um menu. Dado que um menu foi cadastrado, 
  ...
  - Uma chave `pay` que, quando chamada, invoca uma função que soma o valor de todos os pedidos e dá o 
  preço com acréscimo de 10%.
...
// PASSO 2: Adicione ao objeto retornado por `createMenu` uma chave `consumption` que, 
// como valor inicial, tem um array vazio. CHECK
// 
...
const createMenu = (pedidos) => {
  const cardaprio = { fetchMenu: pedidos, consumption: [], };
  cardaprio.order = (string) => cardaprio.consumption.push(string);
  return cardaprio;
};
